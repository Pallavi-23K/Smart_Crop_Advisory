...
def recommend_crop(soil, rainfall_mm):
    ...
    # For each rule that matches the soil, compute a score based on rainfall match.
    scored = []
    for rule in crop_rules:
        if rule["soil"].lower() != soil:
            continue
        rmin = float(rule["rainfall_min"])
        rmax = float(rule["rainfall_max"])
        if rmin <= rainfall_mm <= rmax:
            score = 1.0
        else:
            dist = min(abs(rainfall_mm - rmin), abs(rainfall_mm - rmax))
            score = max(0.0, 1.0 - (dist / 200.0))
        scored.append({"crop": rule["crop"], "score": round(score, 3)})
    ...
    # Fallback suggestions (with decreasing scores)
    soil_suggestions = {
        "clay": [("Paddy (Rice)", 0.9), ("Sugarcane", 0.7), ("Banana", 0.5)],
        "loamy": [("Wheat", 0.9), ("Maize", 0.8), ("Soybean", 0.7)],
        "sandy": [("Millets", 0.85), ("Groundnut", 0.75), ("Cotton", 0.6)]
    }
    suggestions = soil_suggestions.get(soil)
    if suggestions:
        return [{"crop": c, "score": s} for c, s in suggestions]
    return [{"crop": "Mixed vegetables", "score": 0.5}]
